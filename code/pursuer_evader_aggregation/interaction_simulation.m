...
%     d_factor = (centroid_evader_distance <= dth).*(centroid_evader_distance/dth) + (centroid_evader_distance > dth).*(1);
    ...
    attraction_term = reshape(vemax_attraction*repmat(exp(0.01*centroid_evader_distance - K*pursuer_evader_distance),[2,1]).*(centroid_evader_vector./repmat(centroid_evader_distance,[2,1])), 2*number_evader, 1);
