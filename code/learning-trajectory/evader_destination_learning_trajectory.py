...
def get_next_batch(data_x, data_y, step, zero_row_indices):
    ...
    batch_y = np.zeros((batch_size, hp.time_window_size, hp.num_outputs))

    for index in range(batch_size):
        batch_x[index] = data_x[samples_range[0]+index:samples_range[0]+index+hp.time_window_size]
        batch_y[index] = data_y[samples_range[0]+index:samples_range[0]+index+hp.time_window_size]
...
def core_model(inputs, cell_state_sizes, batch_size, keep_prob):
    ...
    pursuer_next_positions = np.zeros((batch_size, hp.time_window_size, 2))
    for i in range(hp.time_window_size):
        rnn_final_output = rnn_outputs[:,i,:]
        ...
        pursuer_next_positions[:,i,:] = predicted_output
    
    return pursuer_next_positions
...
def train_network(train_x, train_y):
    ...
    network_output = tf.placeholder(dtype=tf.float32, shape=[None, hp.time_window_size, hp.num_outputs], name='output_placeholder')   # ground-truth
    ...
    predicted_pursuer_position = tf.reshape(predicted_pursuer_position, [-1, hp.time_window_size, hp.num_outputs])
    pursuer_evader_loss = tf.reduce_mean(tf.reduce_sum(tf.square(tf.subtract(network_output, predicted_pursuer_position)), axis=[1,2]))
    batch_loss = pursuer_evader_loss
